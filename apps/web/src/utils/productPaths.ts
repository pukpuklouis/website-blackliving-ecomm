...
export async function generateStaticPaths(category: string) {
  try {
    ...
    if (!response.ok) {
      console.error('Failed to fetch products for static paths:', response.status, response.statusText);
      // Fallback to hardcoded paths during development
      return [
        { params: { productSlug: `${category}-sample` } }
      ];
    }
    
    const data = await response.json();
    const products = (data && data.success && data.data && data.data.products) ? data.data.products : [];
