...
class MeiliSearchService {
    ...
    async search(options: SearchQueryOptions & { signal?: AbortSignal }): Promise<UnifiedSearchResponse> {
        if (!this.client) {
        ...
        try {
            ...
            const transformToSearchResult = (hit: any): UnifiedSearchResult => ({
                ...
                slug: hit.slug,
                href: hit.href || `/${hit.type === 'product' ? 'products' : hit.type === 'post' ? 'blog' : 'pages'}/${hit.slug}`,
                type: hit.type,
