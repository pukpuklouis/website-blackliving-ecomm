...
type BatchOperationsToolbarProps = {
...
export default function BatchOperationsToolbar({
  ...
  const buildBulkUpdates = (values: typeof bulkEditValues) => {
    const updates: Record<string, string | boolean | number> = {};
    if (values.category && values.category !== "unchanged") {
      updates.category = values.category;
    }
    if (values.inStock !== null) {
      updates.inStock = values.inStock;
    }
    if (values.featured !== null) {
      updates.featured = values.featured;
    }
    if (values.sortOrder) {
      updates.sortOrder = Number(values.sortOrder);
    }
    return updates;
  };
  ...
  const handleBulkEdit = async () => {
    ...
    try {
      setIsProcessing(true);
      const updates = buildBulkUpdates(bulkEditValues);
