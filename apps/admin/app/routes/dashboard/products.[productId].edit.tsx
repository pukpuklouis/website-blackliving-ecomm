...
export const loader = async ({ params, request, context }: any) => {
  ...
  try {
    ...
    if (apiService) {
      ...
      const apiRequest = new Request(`http://internal/api/products/${productId}`, {
        method: 'GET',
      });
    ...
    let categoriesRes;
    if (apiService) {
      const categoriesRequest = new Request('http://internal/api/admin/products/categories', {
        method: 'GET',
      });
...
export default function EditProductPage() {
  ...
  return <ProductEditPage productId={product?.id || undefined} initialData={product || undefined} />;
}
