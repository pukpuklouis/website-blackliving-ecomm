...
describe('Infrastructure Tests', () => {
  ...
  describe('Database Setup', () => {
    it('should initialize database successfully', async () => {
      ...
      const result = await testEnv.db!.prepare('INSERT INTO users (id, name, email) VALUES (?, ?, ?)').bind('test-1', 'Test User', 'test@example.com').run();
    ...
    it('should handle database queries', async () => {
      ...
      // Insert test data
      await testEnv.db!.prepare('INSERT INTO users (id, name, email) VALUES (?, ?, ?)').bind('test-2', 'Test User 2', 'test2@example.com').run();
      
      // Query the data
      const user = await testEnv.db!.prepare('SELECT * FROM users WHERE id = ?').bind('test-2').first();
