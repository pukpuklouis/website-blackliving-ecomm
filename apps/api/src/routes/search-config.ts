...
import { z } from 'zod';
import { saveSearchConfig, getSearchConfig } from '../modules/search';
...
app.post('/config', requireAdmin, zValidator('json', configSchema), async (c) => {
    try {
    ...
    try {
        ...
        if (!config || !config.searchKey) {
            return c.json({
                ...
                message: 'MeiliSearch is not configured or search key is not available',
            }, 503);
        }
