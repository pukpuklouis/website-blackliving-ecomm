...
app.post('/', zValidator('json', businessCooperationCreateSchema), async (c) => {
  try {
    const db = c.get('db');
    ...
    try {
      ...
      return c.json({ success: false, error: 'Verification service unavailable', message: '驗證服務暫時無法使用' }, 503);
    }
    if (!verified) {
      return c.json({ success: false, error: 'Turnstile verification failed', message: '驗證失敗，請重新嘗試' }, 400);
    }
