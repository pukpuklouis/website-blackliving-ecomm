  ...
  try {
    ...
    const customer = {
      ...
      tags: customerResult.tag_names
        ? customerResult.tag_names.split(',').map((name: string, index: number) => ({
      ...
      lastPurchaseAt: customerResult.last_purchase_at
        ? new Date(customerResult.last_purchase_at)
        : null,
      firstPurchaseAt: customerResult.first_purchase_at
        ? new Date(customerResult.first_purchase_at)
        : null,
      lastContactAt: customerResult.last_contact_at
        ? new Date(customerResult.last_contact_at)
        : null,
    };
...
customers.post('/tags', requireAdmin(), zValidator('json', createCustomerTagSchema), async c => {
  try {
    ...
    await c.env.DB.prepare(
      ...
      .bind(crypto.randomUUID(), data.name, data.color, data.description || '', data.category, now)
      .run();
...
customers.post('/tags/assign', requireAdmin(), zValidator('json', assignTagSchema), async c => {
  try {
    ...
    await c.env.DB.prepare(
      ...
      .bind(crypto.randomUUID(), customerProfileId, customerTagId, user?.name || 'admin', now)
      .run();
