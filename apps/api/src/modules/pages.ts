import { and, asc, count, desc, eq, like, or, pages } from "@blackliving/db";
...
app.get("/", zValidator("query", pageQuerySchema), async (c) => {
  try {
    ...
    if (query.search) {
      ...
      conditions.push(
        or(like(pages.title, searchTerm), like(pages.slug, searchTerm))
      );
    }
...
app.put(
  ...
  async (c) => {
    try {
      ...
      if (
        updateData.status &&
        existingPage.status !== updateData.status &&
        (existingPage.status === "published" ||
          updateData.status === "published")
      ) {
        try {
          ...
          if (updateData.status === "published") {
            ...
            await searchModule
              .deleteDocument(existingPage.id)
              .catch((error) => {
