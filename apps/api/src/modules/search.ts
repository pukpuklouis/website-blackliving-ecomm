...
import { products, posts, pages } from '@blackliving/db';
import { transformDocuments, transformDocument } from '../utils/searchSync';
...
const searchConfigSchema = z.object({
    ...
    indexName: z.string().min(1, 'Index name is required').default('blackliving_content'),
});
...
async function generateSearchKey(client: MeiliSearch, indexName: string): Promise<string | null> {
    try {
...
async function saveSearchConfig(c: any, config: SearchConfig): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        if (!client) {
            return { success: false, error: 'Failed to initialize MeiliSearch client' };
        }
        ...
        await cache.set(configKey, fullConfig, 86400 * 30); // 30 days
...
// Reindex all content
async function reindexAll(c: any): Promise<{ indexed: number; errors: string[] }> {
    const result = { indexed: 0, errors: [] };
...
async function indexDocument(c: any, type: string, data: any): Promise<boolean> {
    try {
        // Ensure config is loaded
        if (!currentConfig) {
            await getSearchConfig(c);
        ...
        const document = transformDocument(type as any, data);
...
async function deleteDocument(c: any, type: string, id: string): Promise<boolean> {
    try {
        // Ensure config is loaded
        if (!currentConfig) {
            await getSearchConfig(c);
...
export class SearchModule {
    ...
    async saveConfig(config: SearchConfig): Promise<{ success: boolean; error?: string }> {
    ...
    async updateDocument(document: any): Promise<boolean> {
        return indexDocument(document.type, document);
    }
}

// Export functions for use in other modules
export { saveSearchConfig, getSearchConfig, reindexAll, indexDocument, deleteDocument, getMeiliClient };
