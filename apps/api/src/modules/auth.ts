...
async function checkRateLimit(
  ...
  if (
    ...
    now - existing.firstAttempt < windowSeconds * 1000
  ) {
    return false;
  }
...
authRouter.post("/initiate", zValidator("json", initiateSchema), async (c) => {
  ...
  try {
    ...
    return c.json(
      { success: false, error: "驗證服務暫時不可用，請稍後再試" },
      503
    );
  }
  if (!verified) {
    return c.json(
      { success: false, error: "Turnstile 驗證失敗，請重新操作" },
      400
    );
  }
...
authRouter.post("/callback", zValidator("json", callbackSchema), async (c) => {
  ...
  if (!record) {
    return c.json(
      { success: false, error: "連結已失效，請重新請求 Magic Link。" },
      400
    );
  }
  ...
  if (record.usedAt) {
    return c.json(
      { success: false, error: "此連結已被使用，請重新請求 Magic Link。" },
      400
    );
  }
  ...
  if (record.expiresAt && new Date(record.expiresAt) < now) {
    return c.json(
      { success: false, error: "連結已過期，請重新請求 Magic Link。" },
      400
    );
  }
  ...
  await storeRefreshToken(db, tokens.refreshToken, {
    id: user.id,
    email: user.email,
  });
...
// Note: Custom JWT refresh endpoint removed - using Better Auth only
// All authentication now goes through Better Auth (OAuth, Magic Link)
