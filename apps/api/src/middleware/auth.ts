...
export const requireAdmin = () => {
  return createMiddleware(async (c, next) => {
    ...
    if (user.role !== 'admin') {
      console.warn(`Forbidden admin access attempt: user=${user.id}, role=${user.role}, ip=${ip}, path=${c.req.path}`);
...
export const requireCustomer = () => {
  return createMiddleware(async (c, next) => {
    ...
    if (user.role !== 'customer' && user.role !== 'admin') {
      console.warn(`Forbidden customer access attempt: user=${user.id}, role=${user.role}, ip=${ip}, path=${c.req.path}`);
      throw new HTTPException(403, {
...
export function requireOwnership(userIdParam: string = 'userId') {
  return createMiddleware(async (c, next) => {
    ...
    if (user.id !== resourceUserId) {
      console.warn(`Ownership violation attempt: user=${user.id}, attempted_access=${resourceUserId}, ip=${ip}, path=${c.req.path}`);
      throw new HTTPException(403, {
...
export function requireFreshSession(maxAgeMinutes: number = 30) {
  return createMiddleware(async (c, next) => {
    ...
    if (sessionAge > maxAge) {
      console.warn(`Stale session access attempt: user=${user.id}, session_age=${Math.round(sessionAge / 60000)}min`);
      throw new HTTPException(403, {
