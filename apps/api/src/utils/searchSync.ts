import type { SearchDocument, SearchContentType } from '@blackliving/types';
import type { products, posts, pages } from '@blackliving/db';
...
export function transformProduct(product: ProductRow): SearchDocument {
    ...
    if (Array.isArray(product.variants) && product.variants.length > 0) {
        ...
        if (typeof firstVariant === 'object' && firstVariant && 'price' in firstVariant) {
            price = typeof firstVariant.price === 'number' ? firstVariant.price : undefined;
        }
    }

    // Ensure features is an array
    const tags = Array.isArray(product.features) ? product.features : [];
    const categorySegment = product.category ? toUrlSegment(product.category) : 'uncategorized';
...
export function transformPost(post: PostRow): SearchDocument {
    ...
    const categorySegment = post.category ? toUrlSegment(post.category) : 'uncategorized';
...
export function transformPage(page: PageRow): SearchDocument {
    ...
    // Strip HTML from content for search
    const contentString = typeof page.content === 'string' ? page.content : page.contentMarkdown || '';
 ...
 */
export function transformDocument(type: SearchContentType, data: any): SearchDocument | null {
    try {
        if (!data) {
            console.warn(`Attempted to transform null/undefined data for type: ${type}`);
            return null;
        }
 ...
 */
export function transformDocuments(type: SearchContentType, documents: any[]): SearchDocument[] {
    return documents
...
function isStopWord(word: string): boolean {
    const stopWords = new Set([
        ...
        // Chinese stop words
        '的', '了', '和', '是', '就', '都', '而', '及', '與', '著', '或', '一個', '沒有', '我們', '你們'
    ]);
 ...
 */
export function validateDocument(doc: SearchDocument): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (doc.id && doc.type) {
        ...
        if (!doc.id.startsWith(expectedPrefix)) {
            errors.push(`Document ID should start with "${expectedPrefix}" for type "${doc.type}"`);
        }
    }
